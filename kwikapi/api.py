...
class API(object):
    ...
    def has_api_fn(self, fn_name, version):
        return (version, fn_name) in self._api_funcs

    def get_default_version(self):
        return self.default_version

    def get_api_fn(self, fn_name, version):
        return self._api_funcs[(version, fn_name)]
    ...
    def _resolve_call_info(self, request):
        ...
        if '?' in request.url:
            try:
                ...
                fn_name, query_string = re.findall(r'^/api/(.*)\?(.*)', request.url)[0]
                version = self.api.get_default_version()
        else:
            try:
                version, fn_name = re.findall(r'^/api/(.*)/(.*)', request.url)[0]
            except IndexError:
                fn_name = re.findall(r'^/api/(.*)', request.url)[0]
                version = self.api.get_default_version()

            query_string = ''

        request.fn_name = fn_name
        if not self.api.has_api_fn(fn_name, version):
            raise UnknownAPIFunction(fn_name)

        fninfo = self.api.get_api_fn(fn_name, version)
